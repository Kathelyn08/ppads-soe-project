...
from estudos.models.sessao import Sessao
from datetime import date, timedelta
from django import forms
from django.forms import ModelForm
from django.db.models import Q, F
from ..util import week_range, compact, exclude, timedelta_text
...
class SessaoListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = self.request.user.sessoes.all().order_by('-data_inicial', F('data_final').desc(nulls_first=True))
    ...
    def get_context_data(self, **kwargs):
        ...
        context['progresso'] = sum(map(Sessao.duracao, context['sessoes']), timedelta(0))
        context['progresso_texto'] = timedelta_text(context['progresso'])

        return context

class SessaoCreateView(LoginRequiredMixin, CreateView):
    model = Sessao
    form_class = SessaoForm
    template_name = 'sessao/sessao_form.html'
    success_url = reverse_lazy('sessao-list')

    def form_valid(self, form):
        form.instance.usuario = self.request.user
        return super().form_valid(form)

class SessaoUpdateView(LoginRequiredMixin, UpdateView):
    model = Sessao
    form_class = SessaoForm
    template_name = 'sessao/sessao_form.html'
    success_url = reverse_lazy('sessao-list')
    ...
    def get_queryset(self):
        return self.request.user.sessoes.all()

class SessaoDeleteView(LoginRequiredMixin, DeleteView):
    model = Sessao
    template_name = 'sessao/sessao_confirm_delete.html'
    success_url = reverse_lazy('sessao-list')
